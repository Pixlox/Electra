...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!command) {
			console.error(colours.red('[Electra] [WARNING] ') + `No command matching ${interaction.commandName} was found.`);
			return;
		}
		try {
			await command.execute(interaction);
		} catch (error) {
			console.error(colours.red('[Electra] [WARNING] ') + `Error executing ${interaction.commandName}`);
			console.error(error);
		}
	},
};
